...
- `index_directory`: generates an index file (`index.txt`) for one or more directories, listing each file name, last modification date, and size in MB.
- `pdb2fasta`: extracts sequences in FASTA format from PDB files. Input may be a single file or a directory with PDB files.
...
- `tm_align_hit_extract`: extracts the hit region of sequence 2 from a TM-align output file generated with the `pegi3s/tmtools` Docker image (1.0.1 or above; it may work with other outputs as well). Input may be a single file or a directory with files.
...
- Add the `index_directory` utility.

## [0.23.0] - 04/03/2026

- Add the `tm_align_hit_extract` and `pdb2fasta` utilities.

## [0.22.2-docker29.0.1] - 13/01/2026

- Updates base image to `pegi3s/docker:29.0.1` to make it compatible with current Docker clients.
