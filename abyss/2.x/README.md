...
This image facilitates the usage of [ABySS](https://github.com/BirolLab/abyss), a `de novo` sequence assembler intended for short paired-end reads and large genomes. All `abyss-*` commands are available in the path. For instance, you can show the `abyss-pe` help by running `docker run --rm pegi3s/abyss abyss-pe -h`.
